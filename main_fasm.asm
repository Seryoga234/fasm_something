...
shellcode_start:
    ...
    mov rbx, [gs:60h]  ; gs - это «сын Винды»/Сегмент общего назначения , rbx - база/хранилищем
    ; 60h - это ровно 1 байт (в десятичной это 96 байт)
    mov rbx, [rbx+18h]  ; "+" это смещение как >>/<< в С++
    mov rbx, [rbx+20h]  ;
    mov rbx, [rbx]   ;
    mov rbx, [rbx]  ;
    mov r12, [rbx+20h]  ;
